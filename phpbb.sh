...
instalar_dependencias() {
    ...
    sudo apt install -y unzip apache2 php8.1 php8.1-{mysql,xml,curl,zip,gd,mbstring,xmlrpc,intl,soap} libapache2-mod-php8.1
...
# Función para descargar phpBB
descargar_phpbb() {
    dialog --infobox "Descargando phpBB..." 6 40
    sleep 2
    wget -q https://download.phpbb.com/pub/release/3.3/3.3.13/phpBB-3.3.13.zip

    if [[ $? -ne 0 ]]; then
        dialog --msgbox "Error al descargar phpBB. Verifica tu conexión a Internet." 6 40
        exit 1
    fi

    dialog --infobox "Descomprimiendo phpBB..." 6 40
    sleep 2
